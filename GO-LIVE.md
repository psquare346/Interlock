...
| 8 | Punchout exposure | Token-free by design (SAP calls it), open to localhost | Keep token-free, but restrict at the network layer | IP-allowlist your SAP egress IPs and supplier ranges at the proxy/firewall. For OCI 5.0 extraction, give Basis your static egress IPs (START-HERE §3). Session hooks are already encrypted at rest and single-use. |
...
4. Register the first admin account **before** exposing the URL (first account in a tenant becomes its admin).
